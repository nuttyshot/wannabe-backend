...
class FindAllProductInteractorTest {
  ...
  @Mock
  private ProductsResponsePresenter productsResponsePresenter;

  @Test
  void 전체_상품_조회() {
    // given
    when(findAllProductDsGateway.findAll()).thenReturn(mock(Products.class));
    // when
    interactor.execute();
    // then
    verify(findAllProductDsGateway, only()).findAll();
    verify(productsResponsePresenter, only()).create(any());
  }
}