...
import wannabe.backend.oauth2.interactor.GetOAuth2MemberInteractor;
import wannabe.backend.oauth2.interactor.OAuth2ArgumentChooserService;
import wannabe.backend.oauth2.interactor.OAuth2MemberFactory;
import wannabe.backend.oauth2.usecase.ValidateOAuth2ErrorUseCase;

/**
 * 아래 링크를 통해 code를 얻고 테스트를 실행해야 합니다. <p> code는 일회성 이기 때문에 테스트할 때마다 재발급 받으셔야 합니다. <p>
 * <a href="http://localhost:8080/oauth2/authorization/kakao">kakao</a>
 */
@Disabled("code를 kakao, naver로 부터 얻어서 사용하세요!")
@ExtendWith(MockitoExtension.class)
class GetOAuth2MemberInteractorTest {

  private final OAuth2ArgumentChooserService chooser = mock(OAuth2ArgumentChooserService.class);
  private final OkHttpClient client = new OkHttpClient();
  private final ObjectMapper mapper = new ObjectMapper();
  private final ValidateOAuth2ErrorUseCase errorPort = mock(ValidateOAuth2ErrorUseCase.class);
  private final OAuth2MemberFactory oAuth2MemberFactory = new OAuth2MemberFactory();
  private final GetOAuth2MemberInteractor resolver
      = new GetOAuth2MemberInteractor(chooser, client, mapper, errorPort, oAuth2MemberFactory);
